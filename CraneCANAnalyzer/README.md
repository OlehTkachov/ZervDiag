...
### Подготовка больших Replay TRC

Предварительная проверка выполняется построчно в фоне, показывает прогресс
и отменяется кнопкой ABORT / STOP. Replay также читает кадры потоком, без лимита в 1 000 000 кадров.
Обычный импорт в таблицу сохраняет прежний лимит; память анализа зависит от окон. Подробнее: [Replay](docs/REPLAY_COMPLETION.md).
